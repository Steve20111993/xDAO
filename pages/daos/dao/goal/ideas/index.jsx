...
export default function GrantIdeas() {
		...
		try {
			if (contract && id) {
				...
				for (let i = 0; i < Allvotes.length; i++) {
					...
					if (element.toString().toLocaleLowerCase() === window.selectedAddress.toString().toLocaleLowerCase()) isvoted = true;
				}
				...
				setIdeasURI({
					...
					donation: Number((await contract._ideas_uris(Number(id)).call()).donation) ,
					isVoted: isvoted,
					isOwner: object.properties.wallet.description.toString().toLocaleLowerCase() === window.selectedAddress.toString().toLocaleLowerCase() ? true : false,
	...
	async function VoteIdees() {
		try {
			await sendTransaction(contract.create_goal_ideas_vote(Number(Goalid), Number(id), window.selectedAddress));
		} catch (error) {
			console.error(error);
			return;
		}
			...
			<div className={`${styles.container} flex flex-col items-center gap-8`}>
				<div className={`${styles.title} flex flex-col gap-2`}>
					...
					<div>
						<Loader
							element={
								<a className="font-medium text-[#0000ff]" href={`https://evm-sidechain.xrpl.org/address/${IdeasURI.wallet}`} style={{ color: "var(--title-a-text)" }} rel="noreferrer" target="_blank">
									{IdeasURI.wallet}
								</a>
							}
							width={"80%"}
					...
					<Loader element={<div className="flex">Donated: {IdeasURI.donation} XRP </div>} width={"100%"} />
